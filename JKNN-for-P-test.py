...
p_auc = np.zeros((10,10))
p_acc = np.zeros((10,10))
p_rec = np.zeros((10,10))
p_spe = np.zeros((10,10))
p_gmean = np.zeros((10,10))




for test_j in range(10):
    for test_k in range(10):
        av_auc=[]
        av_acc = []
        av_pre = []
        av_rec = []
        ...
        print("J = ",test_j , " K = ", test_k )
        print("auc :", sum(av_auc) / len(av_auc))
        p_auc[test_j,test_k] = sum(av_auc) / len(av_auc)
        print("acc :", sum(av_acc) / len(av_acc))
        p_acc[test_j, test_k] = sum(av_acc) / len(av_acc)
        print("pre :", sum(av_pre) / len(av_pre))

        print("gmean :", sum(av_gmean) / len(av_gmean))
        p_gmean[test_j, test_k] = sum(av_gmean) / len(av_gmean)
        print("rec :", sum(av_rec) / len(av_rec))
        p_rec[test_j, test_k] = sum(av_rec) / len(av_rec)
        print("spe :", sum(av_spe) / len(av_spe))
        p_spe[test_j, test_k] = sum(av_spe) / len(av_spe)


np.save('JK_p_auc', p_auc)
np.save('JK_p_acc', p_acc)
np.save('JK_p_rec', p_rec)
np.save('JK_p_spe', p_spe)
np.save('JK_p_gmean', p_gmean)
